...
def scrape(title, path):
    ...
    if SCRAPER_MANAGER.getAllPlugins():
        ...
        for plugin in SCRAPER_MANAGER.getAllPlugins():
            click.echo("Searching for movie using the {0} scraper".format(plugin.name))
            click.echo("Using search url: {0}{1}".format(plugin.plugin_object.SEARCH_URL, title))
            ...
            if not search_results:
                ...
                click.echo("Multiple matches have been found by the {0} scraper:".format(plugin.name))
                for index, link in enumerate(search_results):
                    click.echo("  {0} - {1}".format(index, link))
                value = click.prompt('Please enter the number of the link to use', default=0)
        ...
        ET.SubElement(root, "filenameandpath").text = os.path.join(path, title + ".m4v")
